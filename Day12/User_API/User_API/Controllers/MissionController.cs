...
namespace User_API.Controllers
{
    ...
    public class MissionController(IMissionService missionService) : Controller
    {
        ...
        public async Task<IActionResult> AddOrUpdateMission([FromBody] AddMissionRequestModel model)
        {
            ResponseResult result = new ResponseResult();

            try
            {
                string response = await missionService.AddOrUpdateMissionAsync(model);
